...
onecovariate <- function(covxy, varx) {
  covxy / varx
}

twocovariates <- function(varx = 1, vary = 1, varc = 1, covxy, covyc, covxc) {
  num <- covxy*varc - covyc*covxc
  den <- varx*varc - covxc^2
  num / den
}
```
...
```{r}
  ...
  fluidRow(column(12, plotlyOutput("plot1"))),
    fluidRow(
      ...
      column(2,
             numericInput("varx", "$\\sigma^2_{X}$", value = 1))
    )
  )
    ...
    output$plot1 <- renderPlotly({
      ...
      plot2d <- 
        ...
        ggplot(data = data.frame(x = 0), 
               ...
               stat = "function",
               fun = confounderbias,
               mapping = aes(color = "confounderbias")
        ) +
